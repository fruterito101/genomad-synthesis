...
const features = [
  { icon: Dna, title: "DNA Único", description: "8 traits que definen la personalidad de tu agente", color: "var(--color-primary)" },
  { icon: Sparkles, title: "Evolución", description: "Mejora generación tras generación", color: "var(--color-secondary)" },
  { icon: Users, title: "Breeding", description: "Combina agentes para crear nuevas especies", color: "var(--color-accent-1)" },
  { icon: TrendingUp, title: "Rentabilidad", description: "Gana $GMD rentando tus agentes", color: "#F59E0B" },
];
        ...
        <div className="max-w-7xl mx-auto px-4 py-12 sm:py-16 md:py-20">
          ...
          <motion.div className="text-center mb-10 sm:mb-12 md:mb-16" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold mb-4 sm:mb-6 px-2">
              <span style={{ color: "var(--color-text-primary)" }}>Evoluciona tu </span>
            ...
            <p className="text-base sm:text-lg md:text-xl max-w-2xl mx-auto mb-6 sm:mb-8 px-4" style={{ color: "var(--color-text-secondary)" }}>
              El primer protocolo de breeding de agentes AI on-chain. Crea, evoluciona y comercia agentes únicos con DNA verificable.
            </p>
            <LoginButton />
          </motion.div>
          ...
          <motion.div className="rounded-xl p-5 sm:p-6 md:p-8" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-0 mb-6">
              ...
              <div className="flex items-center gap-4">
                <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>{globalAgents.length} agentes activos</span>
              </div>
            </div>
            ...
            {loading ? (
              ...
              <div className="text-center py-8">
                ...
                <p style={{ color: "var(--color-text-secondary)" }}>Sé el primero en conectar tu agente</p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-xs px-2 py-0.5 rounded" style={{ backgroundColor: `${rarity.color}20`, color: rarity.color }}>
                          {rarity.label}
                        </span>
            ...
            {globalAgents.length > 8 && (
              <div className="mt-4 text-center">
                <Button variant="secondary" size="sm" href="/agents">
    ...
    <div className="min-h-screen" style={{ backgroundColor: "var(--color-bg-primary)" }}>
      ...
      <main className="max-w-7xl mx-auto px-4 pt-20 sm:pt-24 pb-8 sm:pb-12">
        ...
        <motion.div className="mb-6 sm:mb-8" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <p className="text-sm sm:text-base" style={{ color: "var(--color-text-secondary)" }}>Tu centro de control para agentes AI evolutivos</p>
        </motion.div>
        ...
        <motion.div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          <motion.div className="p-4 sm:p-6 rounded-xl" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }} whileHover={{ borderColor: "var(--color-primary)" }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm mb-1" style={{ color: "var(--color-text-muted)" }}>Agentes Globales</p>
          ...
          <motion.div className="p-4 sm:p-6 rounded-xl" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm mb-1" style={{ color: "var(--color-text-muted)" }}>Online Ahora</p>
          ...
          <motion.div className="p-4 sm:p-6 rounded-xl" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }}>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm mb-1" style={{ color: "var(--color-text-muted)" }}>Breedings</p>
        ...
        <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          <motion.div className="p-5 sm:p-6 rounded-xl gradient-border" style={{ backgroundColor: "var(--color-bg-secondary)" }} whileHover={{ y: -4 }}>
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-lg sm:text-xl font-bold mb-1 sm:mb-2" style={{ color: "var(--color-text-primary)" }}>Vincula tu Agente</h3>
                <p className="text-sm mb-3 sm:mb-4" style={{ color: "var(--color-text-secondary)" }}>Conecta tu agente OpenClaw y extrae su DNA único</p>
                <Button variant="primary" size="sm" href="/profile">Ir a Profile<ArrowRight className="w-4 h-4" /></Button>
              </div>
            </div>
          </motion.div>
          <motion.div className="p-5 sm:p-6 rounded-xl" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }} whileHover={{ y: -4, borderColor: "var(--color-accent-1)" }}>
            <div className="flex items-start gap-3 sm:gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-lg sm:text-xl font-bold mb-1 sm:mb-2" style={{ color: "var(--color-text-primary)" }}>Inicia Breeding</h3>
                <p className="text-sm mb-3 sm:mb-4" style={{ color: "var(--color-text-secondary)" }}>Combina agentes para crear una nueva generación</p>
                <Button variant="secondary" size="sm" href="/breeding">Explorar<ArrowRight className="w-4 h-4" /></Button>
              </div>
            </div>
          </motion.div>
        </motion.div>
        ...
        <motion.div className="rounded-xl p-5 sm:p-6" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          ...
          {loading ? (
            ...
            <div className="text-center py-8">
              <Star className="w-12 h-12 mx-auto mb-4" style={{ color: "var(--color-text-muted)" }} />
              <p className="mb-4" style={{ color: "var(--color-text-secondary)" }}>Aún no hay agentes conectados</p>
              <Button variant="primary" size="sm" href="/profile">Sé el primero<ArrowRight className="w-4 h-4" /></Button>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4">
                    ...
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs" style={{ color: "var(--color-text-muted)" }}>
                          Gen {agent.generation} {agent.owner?.wallet ? `• ${agent.owner.wallet.slice(0,4)}...` : ""}
                        </p>
                      </div>
                    </div>
