...
function getRarity(traits: Record<string, number>): { label: string; color: string; bg: string } {
  const values = Object.entries(traits)
    .filter(([key]) => key !== 'skillCount')
    .map(([, v]) => v)
    .filter(v => typeof v === 'number');
  if (values.length === 0) return { label: "Común", color: "#6B7280", bg: "rgba(107, 114, 128, 0.15)" };
  const avg = values.reduce((a, b) => a + b, 0) / values.length;
  const max = Math.max(...values);
  
  if (avg >= 80) return { label: "Legendario", color: "#FBBF24", bg: "rgba(251, 191, 36, 0.15)" };
  if (avg >= 75 || max >= 95) return { label: "Épico", color: "#A855F7", bg: "rgba(168, 85, 247, 0.15)" };
  if (avg >= 60 || max >= 85) return { label: "Raro", color: "#3B82F6", bg: "rgba(59, 130, 246, 0.15)" };
  if (avg >= 40) return { label: "Poco Común", color: "#10B981", bg: "rgba(16, 185, 129, 0.15)" };
  return { label: "Común", color: "#6B7280", bg: "rgba(107, 114, 128, 0.15)" };
}

function getTopTraits(traits: Record<string, number>, count = 3) {
  return Object.entries(traits)
    .filter(([key, value]) => typeof value === 'number' && key !== 'skillCount')
    .sort(([, a], [, b]) => b - a)
    .slice(0, count)
    .map(([key, value]) => ({ key, value }));
}

function getSpecialization(traits: Record<string, number>) {
  const entries = Object.entries(traits).filter(([key, value]) => typeof value === 'number' && key !== 'skillCount');
  if (entries.length === 0) return { key: 'technical', label: 'Técnico', color: '#3B82F6' };
...
export default function AgentsCatalog({ initialAgents }: Props) {
      ...
      <main className="max-w-7xl mx-auto px-4 pt-20 sm:pt-24 pb-8 sm:pb-12">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                style={{ backgroundColor: "var(--color-bg-primary)", border: "2px solid var(--color-border)", color: "var(--color-text-primary)" }}
              />
            </div>
            ...
            <div className="relative">
              ...
              {showFilterMenu && (
                <div className="absolute top-full left-0 right-0 sm:right-auto mt-2 rounded-xl overflow-hidden z-20 min-w-[200px]"
                  style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }}>
                  <button onClick={() => { setFilterTrait(null); setShowFilterMenu(false); }}
                  ...
                  {Object.entries(traitConfig).map(([key, config]) => {
                    ...
                    return (
                      <button key={key} onClick={() => { setFilterTrait(key); setShowFilterMenu(false); }}
                        className="w-full p-3 text-left text-sm flex items-center gap-2 hover:bg-[var(--color-bg-tertiary)]"
                        style={{ color: "var(--color-text-primary)", borderBottom: "1px solid var(--color-border)" }}>
                        <Icon className="w-4 h-4" style={{ color: config.color }} /> {config.label}
                      </button>
                    );
                  })}
                </div>
            ...
            <Link href="/breeding">
              <Button variant="primary" size="md" className="w-full sm:w-auto">Ver Todos</Button>
            </Link>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 mb-6">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg" style={{ backgroundColor: "var(--color-bg-secondary)" }}>
            ...
            <span className="text-sm" style={{ color: "var(--color-text-secondary)" }}>{filteredAgents.length} agentes</span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg" style={{ backgroundColor: "var(--color-bg-secondary)" }}>
            <Activity className="w-4 h-4" style={{ color: "var(--color-success)" }} />
            <span className="text-sm" style={{ color: "var(--color-text-secondary)" }}>{filteredAgents.filter(a => a.isActive).length} online</span>
          </div>
        </div>
        ...
        {filteredAgents.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-lg" style={{ color: "var(--color-text-secondary)" }}>No se encontraron agentes</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {filteredAgents.map((agent) => {
              ...
              return (
                <div key={agent.id} className="rounded-xl overflow-hidden cursor-pointer group hover:border-[var(--color-primary)] transition-colors"
                  style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }}>
                  <div className="p-6 pb-4 flex flex-col items-center">
                    <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full flex items-center justify-center mb-4 relative"
                    ...
                    <h3 className="text-lg font-bold text-center mb-1" style={{ color: "var(--color-text-primary)" }}>{agent.name}</h3>
                    ...
                    <span className="px-3 py-1 rounded-full text-xs font-medium flex items-center gap-1"
                      ...
                      <Star className="w-3 h-3" /> {rarity.label}
                    </span>
                  </div>
                  ...
                  <div className="p-4 pt-0">
                    ...
                    <div className="flex items-center justify-between pt-3" style={{ borderTop: "1px solid var(--color-border)" }}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs font-medium" style={{ color: "var(--color-text-secondary)" }}>{specialization.label}</span>
                      </div>
                      <span className="text-xs" style={{ color: "var(--color-text-muted)" }}>Gen {agent.generation}</span>
                    </div>
                  </div>
