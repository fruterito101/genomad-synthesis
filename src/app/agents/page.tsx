...
function getRarity(traits: Agent["traits"]): { label: string; color: string; bg: string } {
  ...
  if (avg >= 80 && spread <= 20) return { label: "Legendario", color: "#FBBF24", bg: "rgba(251, 191, 36, 0.15)" };
...
function getSpecialization(traits: Agent["traits"]): { key: string; label: string; color: string } {
  ...
  const top = entries.sort(([, a], [, b]) => (b as number) - (a as number))[0];
...
export default function AgentsPage() {
  ...
  useEffect(() => {
    console.log("[AgentsPage] Component mounted, fetching agents...");
    
    fetch("/api/leaderboard?limit=50")
      .then(res => {
        console.log("[AgentsPage] Response status:", res.status);
        return res.json();
      })
      .then(data => {
        console.log("[AgentsPage] Got data:", data);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {filteredAgents.map((agent) => {
              ...
              return (
                <div
                  key={agent.id}
                  className="rounded-xl overflow-hidden cursor-pointer group hover:border-[var(--color-primary)] transition-colors"
                  style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }}
                >
