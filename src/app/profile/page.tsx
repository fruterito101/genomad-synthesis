...
export default function ProfilePage() {
  ...
  const generateCode = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/codes/generate", { method: "POST", headers: { "Authorization": `Bearer ${token}`, "Content-Type": "application/json" } });
      const data = await res.json();
      if (!res.ok) { setCodeError(data.error || "Failed"); return; }
      setVerificationCode(data.code); setCodeExpiry(new Date(data.expiresAt));
    } catch (err) { setCodeError(String(err)); }
  ...
  const formatTimeLeft = () => { if (!codeExpiry) return ""; const diff = codeExpiry.getTime() - Date.now(); if (diff <= 0) return "Expired"; return `${Math.floor(diff / 60000)}:${String(Math.floor((diff % 60000) / 1000)).padStart(2, "0")}`; };

  const walletAddress = user?.wallet?.address;
  const shortWallet = walletAddress ? `${walletAddress.slice(0, 6)}...${walletAddress.slice(-4)}` : "";

  if (!ready) {
    return (<div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: "var(--color-bg-primary)" }}><Dna className="w-10 h-10 sm:w-12 sm:h-12 animate-pulse" style={{ color: "var(--color-primary)" }} /></div>);
  }
  if (!authenticated) {
    return (<div className="min-h-screen flex flex-col items-center justify-center gap-4 px-4" style={{ backgroundColor: "var(--color-bg-primary)" }}><Shield className="w-12 h-12 sm:w-16 sm:h-16" style={{ color: "var(--color-primary)" }} /><h1 className="text-xl sm:text-2xl font-bold text-center" style={{ color: "var(--color-text-primary)" }}>Connect to Continue</h1><LoginButton /></div>);
  }
    ...
    <div className="min-h-screen" style={{ backgroundColor: "var(--color-bg-primary)" }}>
      ...
      <main className="max-w-7xl mx-auto px-4 pt-20 sm:pt-24 pb-8 sm:pb-12">
        ...
        <motion.div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 md:p-8 mb-6 sm:mb-8 gradient-border" style={{ backgroundColor: "var(--color-bg-secondary)" }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
            ...
            <div className="flex gap-4 sm:gap-6 md:gap-8 text-center">
              <div><div className="text-2xl sm:text-3xl font-bold" style={{ color: "var(--color-primary)" }}>{agents.length}</div><div className="text-[10px] sm:text-xs" style={{ color: "var(--color-text-muted)" }}>AGENTS</div></div>
              <div><div className="text-2xl sm:text-3xl font-bold" style={{ color: "var(--color-secondary)" }}>{agents.filter(a => a.isActive).length}</div><div className="text-[10px] sm:text-xs" style={{ color: "var(--color-text-muted)" }}>ACTIVE</div></div>
        ...
        <motion.div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
          <Link href="/breeding">
            <motion.div className="p-4 sm:p-6 rounded-xl flex items-center gap-3 sm:gap-4 cursor-pointer" style={{ background: "linear-gradient(135deg, var(--color-primary), var(--color-accent-1))" }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              ...
              <div className="min-w-0"><h3 className="font-bold text-sm sm:text-lg text-white">Start Breeding</h3><p className="text-xs sm:text-sm text-white/80 hidden sm:block">Cross your agents</p></div>
          ...
          <a href="https://testnet.nad.fun/token/0x03DD45bA22F57b715a2F30C3C945E57DA0AC7777" target="_blank" rel="noopener noreferrer">
            <motion.div className="p-4 sm:p-6 rounded-xl flex items-center gap-3 sm:gap-4 cursor-pointer" style={{ background: "linear-gradient(135deg, #F59E0B, #EAB308)" }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              ...
              <div className="min-w-0"><h3 className="font-bold text-sm sm:text-lg text-white">Get GMD</h3><p className="text-xs sm:text-sm text-white/80 hidden sm:block">Buy on nad.fun</p></div>
          ...
          <motion.div className="p-4 sm:p-6 rounded-xl flex items-center gap-3 sm:gap-4 opacity-60 cursor-not-allowed" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }}>
            ...
            <div className="min-w-0"><h3 className="font-bold text-sm sm:text-lg" style={{ color: "var(--color-text-primary)" }}>Marketplace</h3><p className="text-xs sm:text-sm hidden sm:block" style={{ color: "var(--color-text-muted)" }}>Coming soon...</p></div>
          </motion.div>
        </motion.div>
        ...
        <motion.div className="mb-6 sm:mb-8 rounded-xl p-4 sm:p-6" style={{ background: "linear-gradient(135deg, rgba(0, 170, 147, 0.1), rgba(123, 63, 228, 0.1))", border: "1px solid var(--color-secondary)" }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
          <h2 className="text-lg sm:text-xl font-bold mb-3 sm:mb-4 flex items-center gap-2" style={{ color: "var(--color-text-primary)" }}><Link2 className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: "var(--color-secondary)" }} />Link Your Agent</h2>
          <AnimatePresence mode="wait">
            {!verificationCode ? (
              <motion.div key="generate" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <p className="text-sm mb-3 sm:mb-4" style={{ color: "var(--color-text-secondary)" }}>Generate a verification code to link your AI agent.</p>
                <Button onClick={generateCode} disabled={generatingCode} variant="primary" size="sm">{generatingCode ? <><RefreshCw className="w-4 h-4 animate-spin" />Generating...</> : <><Sparkles className="w-4 h-4" />Generate Code</>}</Button>
              ...
              <motion.div key="code" className="space-y-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div>
                  <p className="text-xs sm:text-sm mb-2" style={{ color: "var(--color-text-muted)" }}>Your verification code:</p>
                  ...
                  <p className="mt-2 text-xs sm:text-sm flex items-center gap-1 sm:gap-2" style={{ color: "var(--color-warning)" }}><Clock className="w-3 h-3 sm:w-4 sm:h-4" />Expires: {formatTimeLeft()}</p>
                </div>
                <div className="rounded-lg p-3 sm:p-4" style={{ backgroundColor: "var(--color-bg-tertiary)" }}>
                  <p className="text-xs sm:text-sm" style={{ color: "var(--color-text-secondary)" }}><strong>Next:</strong> Tell your agent:</p>
                ...
                <button onClick={() => { setVerificationCode(null); setCodeExpiry(null); }} className="text-xs sm:text-sm" style={{ color: "var(--color-text-muted)" }}>Generate new</button>
              </motion.div>
            )}
          </AnimatePresence>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
          <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 flex items-center gap-2" style={{ color: "var(--color-text-primary)" }}><Activity className="w-5 h-5 sm:w-6 sm:h-6" style={{ color: "var(--color-primary)" }} />My Agents<span className="text-xs sm:text-sm font-normal px-2 py-0.5 rounded-full" style={{ backgroundColor: "var(--color-bg-tertiary)", color: "var(--color-text-muted)" }}>{agents.length}</span></h2>
          {loading ? (<div className="text-center py-8 sm:py-12"><Dna className="w-10 h-10 sm:w-12 sm:h-12 mx-auto mb-4 animate-pulse" style={{ color: "var(--color-primary)" }} /><p className="text-sm" style={{ color: "var(--color-text-secondary)" }}>Loading...</p></div>
          ) : agents.length === 0 ? (<div className="text-center py-8 sm:py-12 rounded-xl" style={{ backgroundColor: "var(--color-bg-secondary)", border: "1px solid var(--color-border)" }}><Star className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-4" style={{ color: "var(--color-text-muted)" }} /><h3 className="text-lg sm:text-xl mb-2" style={{ color: "var(--color-text-primary)" }}>No agents yet</h3><p className="text-sm px-4" style={{ color: "var(--color-text-secondary)" }}>Link your AI agent above</p></div>
          ) : (
